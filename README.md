...
> ```
>
> 助手会把这些写进本机记录文件（`out/manual-work.md`）并导入流水线，你只需过一眼预览。命令行用户也可以用 `--add` 一句话追加：
>
> ```bash
...
A：见「办公记录也能上账」章节。**最省事的方式是直接告诉你的 AI 助手**——只要「时间 + 干了啥」，格式不用管，它会整理、去重并导入；也可以先用企业微信 / 钉钉 / 飞书的 AI 月度总结导出，或用 `--add` 在命令行一句话追加。
