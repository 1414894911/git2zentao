...
<p align="center">
  ...
  自动变成禅道里结构清晰、工时合理、时间可追溯的任务<br>
  <sub>月底最痛苦的不是写代码，而是回忆自己写过什么。这件事，交给脚本。</sub>
</p>
...
<p align="center">
  <b>Tags：</b> <code>git</code> · <code>zentao</code> · <code>禅道</code> · <code>工时汇报</code> · <code>timesheet</code> · <code>automation</code> · <code>nodejs</code> · <code>playwright</code>
</p>
...
| 需求评审、接口对接、联调支持、写文档——git 里查无此人，干完就忘 | 跟助手说一声「7/24 处理线上告警 3 小时」就导入，**非代码工作同样上账**；写错了 `--sync` 一键对账更正 |
...
> 💬 **流程会自动停下来问你**：在汇总成任务之前，助手会问一句「有没有不在提交记录里的工作要补？给我时间 + 描述就行」——开会、联调、运维、写文档、帮同事解决问题都能补上，**你不用改任何文件**，也不用记格式。
...
git 只能证明「写了代码」；**需求评审与确认、接口/文档对接、联调与测试支持、部署运维、方案与文档编写、跨部门沟通**——这些活真实占用时间，却查无提交，月底最容易漏报。本技能把它们并入同一条流水线。

> 💡 **你不需要自己编辑任何文件。** 直接把「时间 + 干了啥」发给我（你的 AI 助手），格式怎么顺手怎么写，剩下整理、补录、去重我全包：
>
> ```text
> 7/22 开了项目周会，半小时左右
> 7/24 处理线上告警，大概 3 小时，算运维
> 7/30 帮新同事搭环境 1 小时
> ```
>
> 助手会把这些写进本机记录文件（`out/manual-work.md`）并导入流水线，你只需过一眼预览。写错了也不要紧：改完文件（或再 `--add` 一条修正）跑一次 `--sync`，台账会以文件为准增删改。命令行用户也可以用 `--add` 一句话追加：
>
> ```bash
> node scripts/import_manual.js --add "07-24 (3h) [运维] 线上告警处理" --add "07-30 (1h) 新同事环境搭建支持"
> ```

**第 1 步（可选）：让办公软件的 AI 帮你回忆**（企业微信 / 钉钉 / 飞书都自带 AI 工作总结 / 周报助手类功能，把下面的提示词发给它）：
...
**第 2 步：汇到一处**。办公软件导出的结果、自己回忆的零散事项（线下会议、运维值班、线上告警……）都放进同一个文件，格式一致；（不知道写什么格式就直接丢给助手，不用管）：
...
其余两种补录方式（任选）：`--init` 生成带格式说明的模板后手填（示例行已注释，改成自己的记录即可）；或用办公软件的 AI 月度总结导出为纯文本。

```bash
node scripts/import_manual.js --init    # 生成 out/manual-work.md（含格式说明与注释掉的示例行，已存在则不覆盖）
```
...
最省事的方式：**什么都不用整理，直接跟我说**——只要「时间 + 干了啥」：

```text
这个月 7/22 开了项目周会（半小时），7/24 处理线上告警大概 3 小时（算运维），
7/30 帮新同事搭环境 1 小时。帮我把这些补进本月工作记录，然后接着汇总成禅道任务。
```

助手会写入本机记录文件（`out/manual-work.md`）、去重、并入流水线；必要时它会主动问你「还有没有不在提交记录里的工作要补」。

若你更想让办公软件的 AI 先帮忙回忆，也可以先在那边（企业微信 / 钉钉 / 飞书的 AI 月度总结 / 周报助手）跑：
...
```text
我已把办公软件的 AI 月度总结保存到 out/manual-work.md，零散事项也一并写了。
...
A：见「办公记录也能上账」章节。**最省事的方式是直接告诉你的 AI 助手**——只要「时间 + 干了啥」，格式不用管，它会整理、去重并导入；也可以先用企业微信 / 钉钉 / 飞书的 AI 月度总结导出，或用 `--add` 在命令行一句话追加。写错了？改完文件跑 `--sync` 对账，增删改都会同步到台账。
