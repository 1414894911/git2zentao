...
- **办公记录也能上账**：需求评审、接口/文档对接、联调支持、部署运维、会议、运维值班这类非代码工作，用企业微信 / 钉钉 / 飞书的 AI 月度总结一键导入，或手动逐行录入（可标 `(2h)` 人工工时）；任务描述以【办公记录】标注来源，**不伪造 commit hash**。
...
**第 2 步：整理成纯文本**（默认 `out/manual-work.md`，每行一条）。**没在工作软件里留痕的活（线下会议、运维值班、线上告警处理……）也直接手写进同一个文件**，格式完全相同：
...
行格式一览（除了「日期 + 事项」全是可选标注，标注顺序不限）：

| 标注 | 写法 | 作用 |
|---|---|---|
| 模块 | `[运维]` 或 `【运维】`，放事项前 | 归入对应模块子任务，不写归入「通用」 |
| 人工工时 | `(2h)` `(1.5h)` `（2小时）`，任意位置 | 该任务按你标注的工时合计计（**诚实口径**，审计可查）；不写则按难度自动估算 |
| 列表符号 | 行首 `-` `*` `·` 可有可无 | 兼容从聊天窗口直接粘贴 |
| 星期 | `07-22（周一）` 可有可无 | 自动忽略 |

怕写错格式？一键生成模板再往里补：

```bash
node scripts/import_manual.js --init    # 生成 out/manual-work.md（含格式说明与示例行，已存在则不覆盖）
```
...
标注了人工工时的记录（如周会 `(0.5h)`），任务会按标注合计计，不再走难度推算（下面是脚本真实输出）：

```text
【工时依据】
· 难度档位：简单
· 人工指定工时（办公记录）：0.5h + 0.5h = 1h
· 预计工时：1h
```

> **口径与边界**：只导入你本人参与的工作，AI 生成的记录先核对再导入——【办公记录】段是同事可以回查的，编造条目会留下痕迹；未标注工时的评审/沟通/文档/部署类工作默认按「中等」档起算，觉得不合手就按你的团队口径调 `estimate.tierRules`；人工标注的工时同样受单任务上下限与单日上限约束（E1/E2 照常校验）。
...
先把零散内容交给 AI 整理（在任何 AI 助手里，把聊天记录 / 会议纪要 / 运维日志直接贴给它）：

```text
把我本月（07-01 ~ 07-31）在群聊、文档、会议、日报里参与的工作整理成月度工作记录，
按日期逐条输出，每行一条：日期 + 事项描述（做了什么、涉及哪个模块/需求），
可以加 (2h) 人工工时与 [模块名] 标注。重点覆盖：需求功能讨论与确认、接口与文档对接、
联调与测试支持、部署与运维、线上告警处理、会议沟通、方案与文档编写。闲聊不要输出。
```

再把整理结果交给技能（保存到 `out/manual-work.md` 后；线下会议、运维值班这类没进办公软件的活，手动往同一文件里补行即可）：

```text
我已把办公软件的 AI 月度总结保存到 out/manual-work.md（每行一条：日期 + 事项，可带 (2h) 工时与 [模块]）。
用 import_manual 流程 --dry 预览后导入，再重跑 plan_tasks --preview 给我看合并结果；
办公记录要在任务描述里标注【办公记录】，不要伪造 commit hash 冒充代码提交；
标注了工时的任务按人工工时合计计，并保留原始记录供回查。
```
...
A：见「办公记录也能上账」章节——用企业微信 / 钉钉 / 飞书的 AI 月度总结整理成纯文本，`import_manual.js` 导入后与代码提交同流水线汇总；描述里以【办公记录】标注来源。线下会议、运维值班这类没进办公软件的活，直接手动往同一文件里补行（`--init` 可生成模板），还能用 `(2h)` 标注人工工时。
