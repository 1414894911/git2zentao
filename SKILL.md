...
>
> ⚠️ **改工时记录的三条铁律**（实测踩坑）：① 消耗按「+新值 −旧值」增量维护，**同值再提交也会累加**——编辑前想清楚目标值，绝不反复试；② 幽灵消耗（消耗 > 记录之和）不可消除，唯一自救是把记录补到与消耗一致；③ 父任务消耗重算滞后，必要时显式设置（只能增）。详见 references/zentao-ui-pitfalls.md §5.3。
...
| `import_manual.js` | `--init` | 生成记录模板（格式说明 + 已注释的示例行，不覆盖已有文件） |
